...
import { formatDocument, formatRut, validateRut } from '../utils/formatters';
...
export default function PdiDeclaration({ tramite, onSave, onBack }: PdiDeclarationProps) {
  ...
  const handleDocTypeChange = (newType: Tramite['pdi']['docType']) => {
    setDocType(newType);
    setDocNumber(formatDocument(newType, docNumber));
  };

  const handleDocNumberBlur = () => {
    setDocNumber(formatDocument(docType, docNumber));
  };
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const formattedDoc = formatDocument(docType, docNumber);
    if (docType === 'RUT' && !validateRut(formattedDoc)) {
      setError('El RUT chileno ingresado no es válido. Por favor verifique los números y el dígito verificador (ej: 12.345.678-9).');
      return;
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                id="pdi-doctype"
                value={docType}
                onChange={(e) => handleDocTypeChange(e.target.value as Tramite['pdi']['docType'])}
                className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#004a99] transition font-medium bg-white"
              >
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                placeholder={docType === 'RUT' ? 'Ej: 12.345.678-9' : 'Ej: número de pasaporte o DNI'}
                value={docNumber}
